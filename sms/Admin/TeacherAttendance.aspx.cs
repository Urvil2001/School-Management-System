...
public partial class Admin_TeacherAttendance : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (count >= 1)
        {
            ...
            cmd2.CommandText = "DELETE FROM Attendance where date='" + todaydate + "' and rollid='3'";
            cmd2.Connection = con;
        ...
        else
        {
            ...
            {
                string name1 = row.Cells[2].Text;
                string rollno1 = row.Cells[1].Text;
    ...
    private void saveattendance(string id, string name, string dateofclass, string status, string std)
    {
        ...
        cmd.CommandText = "insert into Attendance(name,date,status,std,rollno,rollid) values('" + name + "','" + dateofclass + "','" + status + "','" + std + "','" + id + "','3') ";
